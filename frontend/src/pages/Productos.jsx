...
const calcularMargen = (precioVenta, costoPromedio) => {
  if (!precioVenta || !costoPromedio || costoPromedio <= 0) return null;
  return (((precioVenta - costoPromedio) / costoPromedio) * 100).toFixed(1);
};
...
const Productos = () => {
  ...
  // Cálculos para alertas en tiempo real
  ...
  const margenGanancia = calcularMargen(precioVentaNum, costoPromedioNum);
        ...
        <div style={styles.listaContainer}>
          {productos.length === 0 ? (
            ...
            productos.map((p) => {
              ...
              const margen = calcularMargen(pv, cp);
              const enPerdida = cp > 0 && pv < cp;
              const alerta = infoAlerta[p.id_producto];
              const esCritico = alerta?.nivel === 'critico';
              const esBajo = alerta?.nivel === 'bajo';
              ...
              return (
                <div
                  ...
                  style={{
                    ...styles.productoCard,
                    borderLeft: esCritico ? '3px solid #e53935'
                      : esBajo ? '3px solid #e65100'
                      : 'none',
                    borderRadius: alerta ? '0 12px 12px 0' : '12px',
                  }}
                >
                  {/* Banner de alerta */}
                  {alerta && (
                    <div style={{
                      padding: '8px 18px',
                      backgroundColor: esCritico ? '#fdecea' : '#fff3e0',
                      borderBottom: '1px solid #f0f4f0',
                      fontSize: '12px',
                      color: esCritico ? '#c62828' : '#e65100',
                      fontWeight: '600',
                      display: 'flex',
                      gap: '16px',
                      flexWrap: 'wrap',
                      ...
                      {esCritico && (
                        <span>🚨 Sin stock: {alerta.tallas_sin_stock.map(t => t.talla).join(', ')}</span>
                      )}
                      {alerta.tallas_bajas.length > 0 && (
                        <span>⚠️ Stock bajo: {alerta.tallas_bajas.map(t => `${t.talla} (${t.cantidad})`).join(', ')}</span>
                      )}
                    </div>
                  )}
                  ...
                  <div style={styles.productoFila}>
                    <div style={styles.productoInfo}>
                      <div style={styles.productoNombreRow}>
                        ...
                        {/* 🌟 AQUÍ SE AGREGÓ EL BADGE DE STOCK BAJO GLOBAL DEL PRODUCTO */}
                        {alerta && alerta.tallas_bajas.length > 0 && (
                          <span style={styles.badgeStockBajo}>
                            ⚠️ Stock Bajo
                          </span>
                        )}
                        ...
                        <div style={styles.tallasRow}>
                          {p.tallas.map((t) => (
                            <div
                              ...
                              style={{
                                ...styles.tallaPill,
                                backgroundColor: t.cantidad === 0 ? '#fdecea'
                                  : t.cantidad < 3 ? '#fff3e0'
                                  : '#e8f5ee',
                              }}
                            >
...
const styles = {
  ...
  // 🌟 ESTILO NUEVO PARA EL BADGE GENERAL DE STOCK BAJO
  badgeStockBajo: { padding: '3px 10px', borderRadius: '20px', fontSize: '12px', fontWeight: '600', backgroundColor: '#fff3e0', color: '#e65100' },
